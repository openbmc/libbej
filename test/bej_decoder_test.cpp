...
TEST(BejDecoderPayloadLengthTest, ErrorsOnTrailingBytes)
{
    ...
    // Default lenient policy is the control: same input must still decode.
    BejDecoderJson lenient;
    EXPECT_EQ(lenient.decode(dictionaries, std::span(padded)), 0);

    // Error policy rejects the input as bejErrorInvalidSize.
