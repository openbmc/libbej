...
TEST(BejDecoderResourceLinkTest, DecodeResourceLink)
{
    ...
    std::vector<uint8_t> encodedStream = {
        // PLDM header (7 bytes)
        0x00, 0xF0, 0xF0, 0xF1, // bejVersion (0xF1F0F000 little endian)
        0x00, 0x00,             // reserved
        0x00,                   // schemaClass (major)
        // Root Set (DummySimple, seq=0)
        0x01, 0x00,             // S: nnint(0) -> seq=0
        0x00,                   // F: bejSet (0 << 4 = 0x00)
        0x01, 0x09,             // L: nnint(9) -> value is 9 bytes
        // Set value: element count + child SFLV
        0x01, 0x01,             // element count: nnint(1) -> 1 child
        // Child ResourceLink (Id, seq=1)
        0x01, 0x02,             // S: nnint(2) -> seq=1, schema=primary
        0xE0,                   // F: bejResourceLink (14 << 4 = 0xE0)
        0x01, 0x02,             // L: nnint(2) -> value is 2 bytes
        0x01, 0x2A,             // V: nnint(42) -> PDR ID = 42
    };
...
TEST(BejDecoderResourceLinkTest, DecodeResourceLinkNull)
{
    ...
    std::vector<uint8_t> encodedStream = {
        // PLDM header (7 bytes)
        0x00, 0xF0, 0xF0, 0xF1, // bejVersion
        0x00, 0x00,             // reserved
        0x00,                   // schemaClass
        // Root Set (DummySimple, seq=0)
        0x01, 0x00,             // S: nnint(0) -> seq=0
        0x00,                   // F: bejSet
        0x01, 0x07,             // L: nnint(7) -> value is 7 bytes
        // Set value: element count + child SFLV
        0x01, 0x01,             // element count: nnint(1) -> 1 child
        // Child ResourceLink (Id, seq=1) with null value
        0x01, 0x02,             // S: nnint(2) -> seq=1
        0xE0,                   // F: bejResourceLink (14 << 4 = 0xE0)
        0x01, 0x00,             // L: nnint(0) -> zero length means null
    };
