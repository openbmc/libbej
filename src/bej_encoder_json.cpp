    ...
    const BejDeferredBindingMap& bindings)
{
    ...
    for (const auto& [resourceId, resource] : bindings)
    {
        // Insertion order sets precedence when distinct sources share a value
        // (emplace keeps the first): uri, instanceId, actions, then other
        // parameters. Empty values never produce a placeholder.
        if (!resource.uri.empty())
        {
            reverse.emplace(resource.uri,
                            bejFormatDeferredBinding(
                                bejDeferredBindingResourceLink, resourceId));
        }
        if (!resource.instanceId.empty())
        {
            reverse.emplace(resource.instanceId,
                            bejFormatDeferredBinding(
                                bejDeferredBindingInstanceId, resourceId));
        }
        for (const auto& [actionId, actionUri] : resource.actionUris)
        {
            if (!actionUri.empty())
            {
                reverse.emplace(actionUri, bejFormatDeferredBindingAction(
                                               resourceId, actionId));
            }
        }
        for (const auto& [paramChar, paramStr] : resource.otherParameters)
        {
            if (!paramStr.empty())
            {
                reverse.emplace(
                    paramStr, bejFormatDeferredBinding(paramChar, resourceId));
            }
        }
    }
    return reverse;
                           ...
                           const BejDeferredBindingMap& bindings)
{
    ...
    for (const DeferredBindingRestore& entry : deferredBindingRestores)
    {
        ...
        bejTreeUpdateNodeFlags(node, entry.originalDeferredBinding,
                               node->format.readOnlyProperty,
                               node->format.nullableProperty);
