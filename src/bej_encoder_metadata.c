                                ...
                                uint16_t nodeIndex, uint16_t dictStartingOffset)
{
    ...
    node->leaf.metaData.sflSize = bejNnintSizeOfValue(sequenceNumber);
                                   ...
                                   uint16_t dictStartingOffset)
{
    ...
    node->leaf.metaData.sflSize += bejNnintSizeOfValue(strLenWithNull);
    // V: Bytes used for the value.
                                 ...
                                 uint16_t dictStartingOffset)
{
    ...
    node->leaf.metaData.vSize =
        bejNnintSizeOfValue(enumValueProperty->sequenceNumber);
    // L: Length needed for the value nnint.
    node->leaf.metaData.sflSize +=
        bejNnintSizeOfValue(node->leaf.metaData.vSize);
    return 0;
}
                                     ...
                                     uint16_t dictStartingOffset)
{
    ...
    switch (chNode->nodeAttr.format.principalDataType)
    {
        case bejInteger:
            RETURN_IF_IERROR(
                bejUpdateIntMetaData(dictionaries, parentDictionary, childP,
                                     childIndex, dictStartingOffset));
            break;
        case bejString:
            RETURN_IF_IERROR(
                bejUpdateStringMetaData(dictionaries, parentDictionary, childP,
                                        childIndex, dictStartingOffset));
            break;
        case bejEnum:
            RETURN_IF_IERROR(
                bejUpdateEnumMetaData(dictionaries, parentDictionary, childP,
                                      childIndex, dictStartingOffset));
            break;
        case bejBoolean:
            RETURN_IF_IERROR(
                bejUpdateBoolMetaData(dictionaries, parentDictionary, childP,
                                      childIndex, dictStartingOffset));
            break;
        default:
